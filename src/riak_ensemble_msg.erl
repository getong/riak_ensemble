...
send_all(_Msg, Id, _Peers=[{Id,_}], _Views, _Required) ->
    ...
    riak_ensemble_util:send_event(self(), {quorum_met, []}),
...
send_request({PeerId, PeerPid}, ReqId, Event) ->
    case PeerPid of
        ...
        _ ->
            ...
            riak_ensemble_util:send_event(PeerPid, Event)
    end.
...
reply({riak_ensemble_msg, Sender, ReqId}, Id, Reply) ->
    riak_ensemble_util:send_all_state_event(Sender, {reply, ReqId, Id, Reply}).
...
add_reply(Peer, Reply, MsgState=#msgstate{timer=Timer}) ->
    ...
    case quorum_met(Replies, MsgState) of
        true ->
            cancel_timer(Timer),
            {Valid, _Nacks} = find_valid(Replies),
            riak_ensemble_util:send_event(self(), {quorum_met, Valid}),
...
quorum_timeout(#msgstate{replies=Replies}) ->
    {Valid, _Nacks} = find_valid(Replies),
    riak_ensemble_util:send_event(self(), {timeout, Valid}),
