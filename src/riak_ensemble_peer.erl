...
-compile({pulse_replace_module,
          [{riak_ensemble_util, pulse_riak_ensemble_util}]}).

%% riak_ensemble_util callbacks
...
start_link(Mod, Ensemble, Id, Args) ->
    riak_ensemble_util:start_link(?MODULE, [Mod, Ensemble, Id, Args], []).

-spec start(module(), ensemble_id(), peer_id(), [any()])
           -> ignore | {error, _} | {ok, pid()}.
start(Mod, Ensemble, Id, Args) ->
    riak_ensemble_util:start(?MODULE, [Mod, Ensemble, Id, Args], []).
...
get_leader(Pid) when is_pid(Pid) ->
    riak_ensemble_util:sync_send_all_state_event(Pid, get_leader, infinity).

-spec watch_leader_status(pid()) -> ok.
watch_leader_status(Pid) when is_pid(Pid) ->
    riak_ensemble_util:send_all_state_event(Pid, {watch_leader_status, self()}).

-spec stop_watching(pid()) -> ok.
stop_watching(Pid) when is_pid(Pid) ->
    riak_ensemble_util:send_all_state_event(Pid, {stop_watching, self()}).

-ifdef(TEST).
-spec get_watchers(pid()) -> [{pid(), reference()}].
get_watchers(Pid) when is_pid(Pid) ->
    riak_ensemble_util:sync_send_all_state_event(Pid, get_watchers).
-endif.

get_info(Pid) when is_pid(Pid) ->
    riak_ensemble_util:sync_send_all_state_event(Pid, get_info, infinity).

tree_info(Pid) when is_pid(Pid) ->
    riak_ensemble_util:sync_send_all_state_event(Pid, tree_info, infinity).

backend_pong(Pid) when is_pid(Pid) ->
    riak_ensemble_util:send_event(Pid, backend_pong).

force_state(Pid, EpochSeq) ->
    riak_ensemble_util:sync_send_event(Pid, {force_state, EpochSeq}).
...
debug_local_get(Pid, Key) ->
    riak_ensemble_util:sync_send_all_state_event(Pid, {debug_local_get, Key}).
-endif.
...
forward(Msg, From, State) ->
    catch riak_ensemble_util:send_event(peer(leader(State), State), {forward, From, Msg}),
    {next_state, following, State}.
...
common(tree_pid, From, State, StateName) ->
    riak_ensemble_util:reply(From, State#state.tree),
    {next_state, StateName, State};
...
send_reply(From, Reply) ->
    ...
    riak_ensemble_util:reply(From, Reply),
    ok.
...
do_put_fsm(Key, Fun, Args, From, Self, State=#state{tree=Tree}) ->
    case riak_ensemble_peer_tree:get(Key, Tree) of
        corrupted ->
            ...
            send_reply(From, failed),
            riak_ensemble_util:sync_send_event(Self, tree_corrupted, infinity);
        KnownHash ->
...
do_put_fsm(Key, Fun, Args, From, Self, KnownHash, State) ->
    ...
    case is_current(Local, Key, KnownHash, State2) of
        local_timeout ->
            ...
            %% riak_ensemble_util:sync_send_event(Self, request_failed, infinity),
...
init([Mod, Ensemble, Id, Args]) ->
    ...
    State = #state{id=Id,
                   ...
                   alive=?ALIVE,
                   mod=Mod},
    riak_ensemble_util:send_event(self(), {init, Args}),
...
set_timer(Time, Event, State) ->
    State2 = cancel_timer(State),
    Timer = riak_ensemble_util:send_event_after(Time, Event),
...
cancel_timer(State=#state{timer=Timer}) ->
    %% Note: riak_ensemble_util cancel_timer discards timer message if already sent
    catch riak_ensemble_util:cancel_timer(Timer),
